...
import (
	...
	"github.com/kubeedge/mappers-go/mappers/test01/configmap"
	"github.com/kubeedge/mappers-go/mappers/test01/driver"
	"github.com/kubeedge/mappers-go/mappers/test01/globals"
)

var devices map[string]*globals.Test01Dev
...
func setVisitor(visitorConfig *configmap.Test01VisitorConfig, twin *common.Twin, client *driver.Test01Client) {
...
func onMessage(client mqtt.Client, message mqtt.Message) {
	...
	for twinName, twinValue := range delta.Delta {
		...
		var visitorConfig configmap.Test01VisitorConfig
		...
		setVisitor(&visitorConfig, &dev.Instance.Twins[i], dev.Test01Client)
	}
}

// initTest01 initialize Test01 client.
func initTest01(protocolConfig configmap.Test01ProtocolCommonConfig) (client *driver.Test01Client, err error) {
	// TODO: pass the configuration to the client.
	config := driver.Test01Config{}
	client, _ = driver.NewClient(config)
	return client, nil
}

// initTwin initialize the timer to get twin value.
func initTwin(dev *globals.Test01Dev) {
	for i := 0; i < len(dev.Instance.Twins); i++ {
		var visitorConfig configmap.Test01VisitorConfig
		if err := json.Unmarshal([]byte(dev.Instance.Twins[i].PVisitor.VisitorConfig), &visitorConfig); err != nil {
			klog.Errorf("Unmarshal VisitorConfig error: %v", err)
		...
		twinData := TwinData{Client: dev.Test01Client,
...
func initData(dev *globals.Test01Dev) {
...
func initGetStatus(dev *globals.Test01Dev) {
	getStatus := GetStatus{Client: dev.Test01Client,
...
func start(dev *globals.Test01Dev) {
	var protocolCommConfig configmap.Test01ProtocolCommonConfig
	...
	var protocolConfig configmap.Test01ProtocolConfig
	...
	client, err := initTest01(protocolCommConfig)
	if err != nil {
		klog.Errorf("Init error: %v", err)
		return
	}
	dev.Test01Client = client
...
func DevInit(configmapPath string) error {
	devices = make(map[string]*globals.Test01Dev)
